...
function GetSRDFLunToReplica(device, callback) {
        ...
        async.waterfall([
            ...
            function(callback){ 
                ...
                param['keys'] = ['device','part'];
                param['fields'] = ['srdfgrpn','srdfrgrp','copytype','repltype','replloc','replstat','remarray','remlun','srdfmode','repltech','linkstct','linkstat','tgtstate'];

                if (typeof arraysn !== 'undefined') { 
                    param['filter'] = 'device=\''+arraysn+'\'&(source=\'VMAX-Collector\'&datagrp=\'VMAX-RDFREPLICAS\')';
                } else {
                    param['filter'] = '(source=\'VMAX-Collector\'&parttype=\'LUNtoReplica\')';
                } 
...
function GetAssignedLUNByInitiator(initiators, callback) { 
    ...
    queryString = queryString + " PREFIX  filter:<http://ontologies.emc.com/2015/mnr/topology#>      ";
    queryString = queryString + " PREFIX  rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#>  ";
    queryString = queryString + " SELECT distinct ?arrayName ?poolName ?lunName ?lunRaid   ";
    queryString = queryString + " WHERE {   ";
    queryString = queryString + "       ?mv rdf:type srm:MaskingView .     ";
    queryString = queryString + "       ?mv srm:maskedToInitiator ?initiator .     ";
    queryString = queryString + "       ?mv srm:maskedToDisk ?disk .      ";
    queryString = queryString + "       ?initiator srm:wwn ?init_wwn .  ";
...
function GetAssignedInitiatorByDevices(device, callback) { 
        ...
        queryString = queryString + "     SELECT distinct  ?arraySN ?deviceName ?deviceWWN ?MaskingView  ?initEndPoint  ";
        queryString = queryString + "     WHERE {    ";
        queryString = queryString + "       ?arrayEntity rdf:type srm:StorageEntity .     ";
        queryString = queryString + "       ?arrayEntity srm:serialNumber ?arraySN .    ";
        queryString = queryString + "       ?arrayEntity srm:containsStorageVolume ?device .    ";
