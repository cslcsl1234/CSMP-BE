            ...
            function(result,  callback){ 
                ...
                CallGet.CallGet(param, function(param) {  
                    ...
                    for ( var i in res ) {
                        ...
                        res1[item.device][item.sgname].push(item);
                    }
                   ...
                   for ( var i in result) {
                        ...
                        if ( res1[item.device] !== undefined )
                            if ( res1[item.device][item.sgname] !== undefined )
                                item['sg_member'] =  res1[item.device][item.sgname];
                            else 
                                item['sg_member'] = [];
                        else 
                            item['sg_member'] = [];
        ...
        function ( arg, callback ) { 
            ...
            CallGet.CallGet(param, function(param) { 
                ...
                for ( var i in param.result ) {
                    ...
                    for ( var j in SGList ) {
                        ...
                        if ( item.device == resItem.device && item.sgname == resItem.sgname ) {
                            resItem.SumOfLuns++;
                            var lun = {};
                            lun["lunname"] = item.lunname;
                            lun["SGLunCapacity"] = item.SGLunCapacity;
                            resItem.luns.push(lun);
                ...
                for ( var j in arg ) {
                    ...
                    for ( var jj in SGList ) {
                        var item1 = SGList[jj];
                        if ( item.device == item1.device && item.sgname == item1.sgname ) {
                             
                            item["SumOfLuns"] = item1.SumOfLuns;
                            item["luns"] = item1.luns;
                        }
                    }
                }
                callback(null, arg ); 
 ...
 function GetStorageGroupsPerformance(device, period, start, end, valuetype,  callback) {
        ...
        async.waterfall([
            function(callback){  
                ...
                param['filter_name'] = '(name=\'ResponseTime\'|name=\'ReadRequests\'|name=\'WriteRequests\'|name=\'ReadThroughput\'|name=\'WriteThroughput\')';
                param['keys'] = ['device','part'];
                param['fields'] = ['name','sgname','parttype'];  
                param['filter'] = '(vmaxtype=\'VMAX2\'&datagrp=\'VMAX-StorageGroup\'&parttype=\'Storage Group\')';
 ...
 function GetDirectorPerformance(device, period, start, valuetype, callback) {
        ...
        async.waterfall([
            function(callback){ 
                ...
                param['start'] = start;
                param['type'] = valuetype;
                param['filter_name'] = '(name=\'KBytesTransferred\'|name=\'CurrentUtilization\'|name=\'ReadRequests\'|name=\'WriteRequests\'|name=\'ReadThroughput\'|name=\'WriteThroughput\')';
                param['keys'] = ['device','part'];
...
function getArrayPerformance(callback) {
 ...
 function getArrayPerformance1(callback) {
