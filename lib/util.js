...
exports.getlastMonthByDate = function (currentDate) {
    ...
    var firstday = new Date(lastday.getFullYear(), lastday.getMonth() - 1, 2);
...
exports.convertSRMPerformanceStructV1 = function (originPerfData) {
    ...
    console.log("TEST3.1  " + oriArray.length);
    for (var i in oriArray) {
        var item = oriArray[i].properties;
        //console.log(item.part + '\t' + item.name);
        var matrics = oriArray[i].points;
        ...
        if (isfind == false) {
            ...
            console.log("TEST3.4  " + i);
            result.push(item);
        }
        console.log("TEST3.5  " + i);
