...
function GetApplicationInfo( callback ) {
    ...
    var appTopoFile = ReportOutputPath + '//' + 'applicationInfo.json';
...
function initiatalApplicationInfo( callback ) {
    ...
    //console.log(xlsFileName+'  '+path.basename(CMDB_FILE_NAME,path.extname(CMDB_FILE_NAME)));
...
function ArrayAccessInfosTEST(device, callback) {
    ...
    async.waterfall([
        ...
        function(  callback){  
            console.log("=====================================");
            console.log(Date() + " ---  VPLEX GetStorageView is begin ---"); 
             var arg = [];
        ...
        console.log(" --- ArrayAccessInfos return  is begin ---");
...
function E2ETopology(device, callback) {
    ...
    async.waterfall([
        ...
        function(arrayInfo, callback) { 
            console.log(Date()+" --- GetMaskViews is begin ---");
            ArrayAccessInfosTEST(device, function(res) { 
        ...
        function (arg , callback) {
            var fabric;
            console.log(Date()+ " -------- getZone info is begin ------");
           SWITCH.getZone(fabric, function(zonelist) { 
                ...
                arg["zone"] = zonelist;
                console.log(" -------- getZone info is done ------");
                callback(null,arg);
           });
        ...
        console.log(" --- E2ETopology return  is begin ---");
...
function getAppStorageRelationV2 (device, callback) {  
    ...
    async.waterfall(
        [
			...
			if ( item.sgname == 'ECAS_RACRW_SG' ) console.log(item);
            ...
            function ( arg1, callback ) {
                ...
                initiatalApplicationInfo( function (ret ) {
                    ...
                    for ( var i in ret ) {
                        var item = ret[i];
                        if ( item.WWN == "" ) 
                            console.log("Warning: hbawwn is EMPTY!" + JSON.stringify(item));
                        else  if ( appinfo[item.WWN] === undefined )
                            appinfo[item.WWN] = item;
                        else 
                            console.log("Warning: hbawwn is dupultant!" + JSON.stringify(appinfo[item.WWN]));
			...
			if ( sginfo.sgname == 'ECAS_RACRW_SG' ) console.log(sginfo);
        ...
        function( arg1 , callback ) { 
            ...
            getVMAXDirectorAddress( device, function ( address ) {
                ...
                for ( var i in address ) {
                    ...
                    //console.log(arg1[item.device].maxlundirector+"\t"+arg1[item.device].device_name);
