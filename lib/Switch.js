...
function getZone(fabwwn, callback) { 
    ...
    [
        ...
        function(callback){
            var deviceid;
            console.log(Date() + '------ GetSwitchPorts is begin ------');
        ...
        function(param,  callback){ 
            var zoneResult = [];
            console.log(Date() + '------ getFabric is begin ------');
            getFabric(fabwwn,function(resultJson) {

                console.log('#ZoneMember='+resultJson.length + '; #SwitchPort=' + param.length);
                var fabricResult= {} ;
                for ( var i in param) {
                    var swport = param[i]; 
                   ...
                   // if ( fabricResult[keyPortZone] !== undefined ) console.log("PortZone:"  + keyPortZone);
                   // if ( fabricResult[keyWWNZone] !== undefined ) console.log("WWNZone:"  + keyWWNZone);
 
                    fabricResult[keyPortZone]=swport;
                ...
                for ( var i in resultJson ) {
                    ...
                    switch (item.zmemtype) {
                        case "Switch Port ID": 
                                    var swport = fabricResult[keyPortZone];
                    ...
                    if ( i % 1000 == 0  ) console.log(Date() + " finished " + i + ', total ' + resultJson.length);
...
function GetSwitchPorts(device, callback) {
    ...
    async.waterfall([
        function(callback){ 
            ...
            param['keys'] = ['device','partwwn']; 
            param['fields'] = ['part','partid','vendor','porttype','partwwn','partstat','partphys','ip'];
                ...
                for ( var i in arg1 ) {
                    ...
                    for ( var j in param.result ) {
                        ...
                        if ( portItem.device == connItem.device && portItem.partwwn == connItem.partwwn ) {
                            ...
                            var json = xml2json.toJson(connItem.connecte, options); 
                            var nodeinfos = json.nodeinfo[0].node ; 
                                for ( var j in nodeinfos ) { 
                                    var nodeItem = nodeinfos[j] ;
        
                                    //console.log("nodeItem="+JSON.stringify(nodeItem));
