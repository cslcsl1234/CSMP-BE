...
async function createProcessWorker() {
    ...
    zbc.createWorker({
        taskType: 'Pre-VPLEX-Provider-Service',
        ...
        taskHandler: (job, complete, worker) => {
            ...
            switch (request.ProtectLevel.hostDeplpy) {
                case "SC":
                    ...
                    var viewItem = { cluster: clusterNamesString, viewname: ViewName }
                    AssignStorageViews.push(viewItem);
                    break;
                ...
                case "SC+TC":
                    ...
                    var ViewName = VPLEX.GenerateStorateViewName(request, "prod");
                    var viewItem = { cluster: "cluster-1", viewname: ViewName }
                    AssignStorageViews.push(viewItem);

                    var ViewName = VPLEX.GenerateStorateViewName(request, "TC");
                    var viewItem = { cluster: "cluster-2", viewname: ViewName }
                    AssignStorageViews.push(viewItem);
                    break;
            }
    ...
    zbc.createWorker({
        taskType: 'Post-VPLEX-Provider-Service',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-Initial-Configure-Info',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-verification-configuration',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
            ...
            async.waterfall([
                function (callback) {
                    ...
                    for (var z in AssignStorageViews) {
                        ...
                        var clustername = assignSVItem.cluster;
                        var viewname = assignSVItem.viewname;
                        ...
                        if (isfind == false) {
                            var msg = `the assign storage view [${viewname}] not exist in cluster [${clustername}] in VPLEX!`;
                            worker.log(msg);
                            callback(501, msg);
                        }
                    }
    ...
    zbc.createWorker({
        taskType: 'VPLEX-CreatePhysicalVolume',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
        onReady: () => zbWorker.log('Connected.'),
        taskHandler: (job, complete, worker) => {
            logger.info("VPLEX-CreatePhysicalVolume is begin.")
            var paramaters = job.variables;
    ...
    zbc.createWorker({
        taskType: 'VPLEX-REDiscoveryPhysicalArray',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-ClaimPhysicalVolumes',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-CreateLocalDevice',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-CreateDistributDevice',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-CreateVirtualVolume',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-AssignStorageView',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
        ...
        taskHandler: (job, complete, worker) => {
            ...
            for (var clustername in vvols) {
                for (var z in AssignStorageViews) {
                    var assignSVItem = AssignStorageViews[z];
                    var svCluster = assignSVItem.cluster;
                    var svName = assignSVItem.viewname;

                    if (clustername == svCluster) {
                        var AssignStorageViewParamater = {
                            StepGroupName: stepgroupname,
                            Step: '分配虚拟化存储卷至存储视图（Storage View）:' + svName,
    ...
    zbc.createWorker({
        taskType: 'VPLEX-AssignConsistencyGroup',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-AssignStorageView-Prod-SC',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-AssignStorageView-Prod-TC',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
    ...
    zbc.createWorker({
        taskType: 'VPLEX-AssignStorageView-Backup',
        maxJobsToActivate: 32,
        loglevel: 'NONE',
        timeout: Duration.seconds.of(301), // 30s timeout
        onConnectionError: () => zbWorker.log('Disconnected'),
