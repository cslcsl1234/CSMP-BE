...
async function createProcessWorker() {
    ...
    zbc.createWorker('VPLEX-verification-configuration-worker', "VPLEX-verification-configuration", (job, complete, worker) => {
        var paramaters = job.variables;
        var VPLEXProviderParamater = paramaters.subParamater;
        var inputParamaters = VPLEXProviderParamater.Paramaters;
    ...
    zbc.createWorker('VPLEX-CreatePhysicalVolume-worker', "VPLEX-CreatePhysicalVolume", (job, complete, worker) => {
        ...
        for (var i in inputParamaters.Volumes) {
            ...
            if (volumeItem.type == "distributed" && clusterNames.length != 2) {
                var msg = "The virtual volume type is distributed, but #cluster is not 2!";
                var code = 501;
                worker.log(msg);
                return complete.failure(msg);
            }
    ...
    zbc.createWorker('VPLEX-CreateDistributDevice-worker', "VPLEX-CreateDistributDevice", (job, complete) => {
        var paramaters = job.variables;
    ...
    zbc.createWorker('VPLEX-CreateVirtualVolume-worker', "VPLEX-CreateVirtualVolume", (job, complete) => {
        var paramaters = job.variables;
