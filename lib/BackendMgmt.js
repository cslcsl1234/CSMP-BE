...
function testCollectObject(testinfo, callback) {
    ...
    async.waterfall(
        [
            function (callback) {
                ...
                BackEndLogin(function (BEInfo) {
                    ...
                    if (exeType == 'delete') {
                        ...
                        logger.info(JSON.stringify(requireForm,2,2));
            ...
            logger.info("====== Device Test operate is completed  ======");
...
function chooseJSONAnswer(objQuery, deviceinfo) {
    ...
    switch (objQuery.exportId) {
        case "vmax":
            jsonAnswersStr = combineRequestAnswer_vmax(objQuery, deviceinfo);
            break;
        case "unisphere":
...
function combineRequestAnswer_vmax(obj, deviceinfo) {
    ...
    unisphereInfo.useAdvancedSettings = deviceinfo["vmax.unisphere.useAdvancedSettings"];
    unisphereInfo.maxValues = deviceinfo["vmax.unisphere.maxValues"];

    var jsonAnswers = {};
    jsonAnswers.vmax_device_type = deviceinfo["vmax.vmax_device_type"];
    jsonAnswers.serialnb = deviceinfo["vmax.serialnb"];
    if ( obj.spbVersion == '4.1.1' ) {
