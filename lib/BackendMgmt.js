...
function testCollectObject(testinfo, callback) {
    ...
    var jsonAnswersStr = chooseJSONAnswer(query["export-id"], testBody.deviceinfo);
    /*
    switch (query["export-id"]) {
        ...
        case "discoverycontext":
            var jsonAnswersStr = combineRequestAnswer_brocadeV2(testBody.deviceinfo);
            break;

        case "vplex":
            var jsonAnswersStr = combineRequestAnswer_vplex(testBody.deviceinfo);
            break;

        case "emcisilon.system":
            var jsonAnswersStr = combineRequestAnswer_isilon(testBody.deviceinfo);
            break;

        default:
            console.log("Not support test device type [" + query["export-id"] + "].")
            callback({});
            break;
    }
    */
            ...
            function (callback) {
                ...
                getCollectObjects(objQuery, function (result) {
                    ...
                    for (var i in result) {
                        ...
                        var jsonAnswersStr = chooseJSONAnswer(objQuery.exportId, deviceinfo);
                        /*
...
function chooseJSONAnswer(value, deviceinfo) {
    var jsonAnswersStr = {};
...
function combineRequestAnswer_ecs(deviceinfo) {
    ...
    var jsonAnswers = {
        ...
        "collect_ns_and_bucket"	 : deviceinfo["ecs.collect_ns_and_bucket"]
    }
