            ...
            function (callback) {
                ...
                async.mapSeries(hostinfo, function (hostinfoItem, subcallback) {
                    ...
                    async.waterfall(
                        [
                            function (callback1) {
                                var cmd = "switchshow; version";
                                if (hostinfoItem.rbash == true) cmd = `rbash --login -c '${cmd}'`;
                                //logger.info(cmd);
                                SSH.remoteCommand(hostinfoItem, cmd, function (output) {
                                    //logger.info("|"+JSON.stringify(output,2,2)+"|");
        ...
        [
                ...
                async.mapSeries(clihostinfo, function (clihostinfoItem, subcallback1) {
                    ...
                    async.mapSeries(sphostinfo, function (spItem, subcallback2) {
                        ...
                        SSH.remoteCommand(clihostinfoItem, cmd, function (xmloutput) {
                            ...
                            if (xmloutput.code !== undefined) {
                                //subcallback2(xmloutput.code, xmloutput);
                                var json2 = { "clihostinfo": clihostinfoItem, "spinfo": spItem, "output": xmloutput }
            ...
            function (outputJson, callback) {
                ...
                for (var i in outputJson) {
                    ...
                    //logger.info("-------\n"+JSON.stringify(item,2,2));
                    if (item.output.code === undefined) {
                        ...
                        var detailInfoItem = { "name": "", "status": "" }
                        detailInfoItem.name = "info";
                        detailInfoItem.status = item.output.data;
                        newItem.detail.push(detailInfoItem);
            ...
            function (arg1, callback) {
                for (var j in arg1) {
                    ...
                    for (var i in data2) {
                        ...
                        if (item.indexOf('health_check_full_scan') >= 0) continue;
        ...
        [
            function (callback) {
                ...
                async.mapSeries(hostinfo, function (hostinfoItem, subcallback1) {
                    ...
                    XIO.HealthCheck(hostinfoItem, startdatetime, enddatetime, function (result) {
                        var result1 = { arrayinfo: hostinfoItem }
                        if (result.code == 200) {
                            var data = [];
                            ...
                            result1['data'] = result;
                            subcallback1(result.code, result1);
                        }
            ...
            function (arg1, callback) {
                ...
                for (var i in arg1) {
                    ...
                    var arrayName = arrayItem.arrayinfo.name;
                    ...
                    for (var j in arrayItem.data) {
                        var item = arrayItem.data[j];
                        switch (item.severity) {
                            case 'major':
                                resultItem.summary.major++;
                                resultItem.detail.push(item);
                                break;

                            case 'critical':
                                resultItem.summary.critical++;
                                resultItem.detail.push(item);
                                break;
            ...
            if (err) callback(result);
                ...
                for (var i in result) {
                    var item = result[i];
                    outputResult.push(item.summary);
                    if ((item.summary.major + item.summary.critical) > 0)
                        detailsheets[item.summary.arrayname] = item.detail;
                }
                var wb = XLSX.utils.book_new();
                var ws = XLSX.utils.json_to_sheet(outputResult);
                XLSX.utils.book_append_sheet(wb, ws, "Health Check Summary");
