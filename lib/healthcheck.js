        ...
        [
            function (callback) {
                ...
                async.mapSeries(hostinfo, function (hostinfoItem, subcallback) {
                    SSH.remoteCommand(hostinfoItem, "symcfg list -output xml", function (result) {
                        var options = {
                            object: true
                        };

                        var xmloutput = result.data;
            ...
            function (tasklist, callback) {
                async.mapSeries(tasklist, function (taskItem, subcallback) {
                    ...
                    SSH.remoteCommand(hostinfoItem, cmd, function (result) {
                        var options = {
                            object: true
                        };
                        var xmloutput = result.data;
            ...
            function (callback) {
                ...
                async.mapSeries(hostinfo, function (hostinfoItem, subcallback) {
                    ...
                    async.waterfall(
                        [
                            function (callback1) {
                                SSH.remoteCommand(hostinfoItem, "switchshow; version", function (result) { 
                            ...
                            function (switchinfo, callback1) {
                                ...
                                SSH.remoteCommand(hostinfoItem, cmd, function (result) {
                                    //console.log(output);
                                    var output = result.data;
                                    switchinfo["HealthStatus"] = /SwitchState:[\t ]*(.*?)\n/.exec(output)[1]
        ...
        [
            function (callback) {
                ...
                async.mapSeries(clihostinfo, function (clihostinfoItem, subcallback1) {
                    ...
                    async.map(sphostinfo, function (spItem, subcallback2) {
                        ...
                        console.log(cmd);
                        console.log(spItem);
                        console.log(clihostinfoItem);
                        SSH.remoteCommand(clihostinfoItem, cmd, function (result) {
                            var options = {
                                object: true
                            };
                            if ( result.code == 200 ) {
...
=======
            ...
            function(result, callback ) {
                ...
                for (var i in result) {
                    var item = result[i];
                    var newItem = {};
                    newItem["sp_ip"] = item.sp_ip;
                    newItem["subsystem"] = item.Subsystem;
                    newItem["HealthStatus"] = item.healthStatus; 
                    
                    summary.push(newItem);
    
                    if (item["healthStatus"] == 'Faulted') {
                        detail[item.Subsystem] = item.detail;
                    }
                }
                // Summary 
                var wb = XLSX.utils.book_new();
