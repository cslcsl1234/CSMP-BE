...
var RecordFlat = require('./RecordFlat');
...
*/
    ...
    async.waterfall([
        function(callback){ 
            var f = new DoGet(getParamater.fields , getParamater , callback);
 
        },        
        function(arg1,result,  callback){ 
            var f = new DoGet(arg1, getParamater, callback);
...
function DoGet(fields, paramater , callback) { 
            ...
            unirest.get(config.Backend.URL + getMethod )
                    ...
                    .end(function (response) { 
                        if ( response.error ) {
                            ...
                            if ( isFlat ) 
                                var resultRecord = RecordFlat(response.body, paramater.keys); 
                            else 
