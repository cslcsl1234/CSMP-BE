        ...
        async.waterfall([
            function(callback){ 
                ...
                CallGet.CallGetPerformance(param, function(param) { 
                    for ( var i in param ) {
                        var item = param[i];
                        item["TotalBandwidth"] = item.matricsStat.TotalBandwidth.max;
                        item["TotalThroughput"] = item.matricsStat.TotalThroughput.max;
                        item["ResponseTime"] = item.matricsStat.ResponseTime.max;
                        item["QueueLength"] = item.matricsStat.QueueLength.max;
            ...
            function ( arg, callback ) { 
                ...
                CallGet.CallGetPerformance(param, function(param) {  
                    for ( var i in arg ) {
                        ...
                        for ( var j in param ) {
                            var item1 = param[j];
                            if ( item.device == item1.device && item.uid == item1.uid ) { 
            ...
            function(arrayinfo, callback) { 
                ...
                topos.querySparql(queryString, function(result) { 
                    ...
                    for ( var i in result ) {
                        ...
                        viewItem["array_model"] = arrayinfo[viewItem.device].model;
                        viewItem["sn"] = arrayinfo[viewItem.device].serialnb;
                        viewItem["ip"] = arrayinfo[viewItem.device].ControllerIP;
                       ...
                       // portgrp_member_item["device"] = item.TARGET_ID.split(':')[2];
                        portgrp_member_item["device"] = arrayinfo[viewItem.device].serialnb;
