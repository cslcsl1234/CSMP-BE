          ...
          App.GetApps( function( app_code, app_result ) {
             var result = []; 
              for ( var i in res) {
                ...
                var apps_name = '';
                // Apps 
                for ( var z=0; z<item.APPs.length; z++) {
                  var appID = item.APPs[z];
                  ...
                  for ( var j in app_result ) {
                     ...
                     if ( appID == appItem.id ) {
                        ...
                        apps.push(appItem);
                        if ( apps_name == '' )
                            apps_name = appItem.name;
                        else 
                            apps_name = apps_name + ',' + appItem.name;
                     }
                  }
                }
                  item.APPs = apps;
                ...
                for ( var hbaj =0 ; hbaj < item.HBAs.length; hbaj++) {
                  ...
                  newRecord['managementip'] = item.baseinfo.management_ip;
                  
                  if ( item.configuration === undefined ) item["configuration"] = { OS: '', OSVersion: '', memory: '', cpu: '', other: '' };
...
function GetHosts(device, callback) {
        ...
        if ( typeof device !== 'undefined') {
            var hostname = device;
            HostObj.findOne({"baseinfo.name" : hostname },{"_id":0, "__v":0}).lean().exec( function (err, doc) {
                //system error.
                if (err) { 
            ...
            HostObj.find({},{"_id":0, "__v":0}).lean().exec( function (err, doc) {
                //system error.
                if (err) { 
                    callback(500 , {status: err})
                }
