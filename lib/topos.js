...
exports.querySparql = function(queryString, callback) {
    ...
    async.mapSeries(config.BackendExtra, function(backendItem, callback) {
        ...
        TaskSerial++;
        logger.info(`Task(${TaskSerial}) APG-SERVER-INFO: ${JSON.stringify(backendItem)}`);
        ...
        req.end(function(res) {
            if (res.error) {
                ...
                logger.info(`Task(${TaskSerial}) finished: ${result.length}`);
        ...
        if ( resItem !== undefined )
            logger.info("#FinalRecords=[" + resItem.length + "]");
        logger.info(" ============================================================================================ ");
    ...
    for (var i_array in arrayports) {
        ...
        for (var i_sw in swports) {
            ...
            if (swConnPortWWN == arrayPortWWN) {
                ...
                //logger.info(swportItem.device + '\t|' + swportItem.part + '\t|' +swportItem.partwwn + '\t|' + swportItem.portwwn+ '\t|' + arrayportItem.device + '\t|' + arrayportItem.feport + '\t|' + arrayPortWWN);
...
function transformTopoDataL2(apptopo) {
    ...
    for (var key in entityList) {
        ...
        //logger.info(`displayName=${item.displayName},textLength=${textLength}, portLength=${portLength}`);
