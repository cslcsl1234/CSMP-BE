...
exports.getEntitys = function (callback) { 
        ...
        console.log(config.Backend.TopoServURL + "/sparql/query");
        unirest.post(config.Backend.TopoServURL + "/sparql/query" ) 
                ...
                .end(function (response) { 
                    if ( response.error ) {
                        ...
                        var entitysJson = JSON.parse(response.body);  

                        var resJson = combineEntity(entitysJson);
        ...
        unirest.post(config.Backend.TopoServURL + "/sparql/query" ) 
                ...
                .end(function (response) { 
                    if ( response.error ) {
                        ...
                        //var resultRecord = RecordFlat(response.body, keys);
                        var linksJson = JSON.parse(response.body);   
                        callback(linksJson);
                    }
        }); 
...
function combineEntity( entitysJson ) {
    ...
    for ( var i in resultJson ) {
        ...
        if ( item.category == "StorageEntity" || item.category == "PhysicalSwitch"  ) {
            ...
            if ( groupBy == "Cisco Systems" ) groupBy = "Cisco";
            item["group"] = groupBy;
