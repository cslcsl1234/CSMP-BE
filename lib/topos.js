...
exports.CombineLunTopoViews = function (masking, apptopoview ) { 
    ...
    for ( var i in masking ) {
        ...
        for ( var j in sgDevices ) {
            ...
            for ( var z in item.initgrp_member ) {
                var initItem = item.initgrp_member[z];

                if ( devItem.lunwwn === undefined ) continue;

                for ( var ii in apptopoview ) {
                    var topoItem = apptopoview[ii];
                    if ( topoItem.arraytype != 'middle' ) continue;
                    //console.log(topoItem.array +'\t'+ devItem.serialnb +'\t'+ topoItem.maskingview +'\t'+ devItem.sgname)
                    if ( topoItem.array == devItem.serialnb && topoItem.maskingview == devItem.sgname ) {
                        var lunTopoItem = {};
                        lunTopoItem["device"] = item.device;
                        lunTopoItem["serialnb"] = devItem.serialnb;
                        lunTopoItem["sgname"] = devItem.sgname;
                        lunTopoItem["lunwwn"] = devItem.lunwwn; 
                        lunTopoItem["lunid"] = devItem.part;
                        
                        lunTopoItem["lunname"] = devItem.lunname;
                        lunTopoItem["memberof"] = devItem.memberof;
                        lunTopoItem["Capacity"] = devItem.Capacity;
                        ...
                        lunTopoItem["ConnHBAWWN"] = initItem;
        
                        lunTopoViews.push(lunTopoItem);
                    }
                }
