                    ...
                    switch (arrayType) {
                        case 'VMAX':
                            ...
                            for (var j = 1; j <= request.count; j++) {
                                ...
                                var volName = VMAX.GenerateVolName(item, request, timestamp) + volno + '_cdplocal';
                                ...
                                var localDeviceName = "device_" + volName;
                                var createLocalDeviceParamater = {
                                    method: 'CreateLocalDevice',
                                    DependOnAction: "CreateExtent",
                                    devicename: localDeviceName ,    // Need matche "Device Naming Rule"
                        ...
                        case 'Unity':
                            ...
                            for (var j = 1; j <= request.count; j++) {
                                ...
                                var volName = UNITY.GenerateVolName(item, request, timestamp) + volno + '_cdplocal';
                                var CreateArrayDeviceParamater = {
...
function CapacityProvisingServiceV3 ( AutoObject, maincallback ) {
    ...
    async.waterfall(
        [
            ...
            function( arg1, callback ) {
                CreateProdVolume(AutoObject,function(callback) {
    ...
    */
    ...
    async.waterfall(
        [
            ...
            // 2.7 Review or execute action in VPLEX metro base on the 2.6 paramaters
            function (AutoObject, callback) {
                autologger.logger.info("Begin execute step [ Execute Automateion function for each action ] !");
                // -----------------------------------------------------------
                // Execute Automateion function for each action
                ...
                if (opsType !== undefined && opsType == "review") {
                    ...
                    ExecuteActions(executeParamaters, arrayInfo, ws, function (result) {
                        AutoObject.ActionResponses = result;
                        callback(null, AutoObject);
                    })
