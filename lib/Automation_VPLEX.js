            ...
            function (AutoObject, callback) { 
                AutoObjectCheckVaild(AutoObject, function (retAutoObject) {
                    if (retAutoObject.resMsg.code != 200) {
                        //retAutoObject.resMsg.code = 500;
                        //retAutoObject.resMsg.message.push("AutoObjectCheckVaild is FAIL!");
...
function ComponeAutoInfo(AutoObject, maincallback) {
    ...
    async.waterfall(
        [
            ...
            , function (AutoObject, callback) {
                autologger.logger.info("Begin execute step [ ComponeAutoInfo.GetStorageViewByProtectLevel for DR_SameCity ] !");
                // ----------------------------
                ...
                if (request.ProtectLevel.DR_SameCity == true) {
                    GetStorageViewByProtectLevel(AutoObject, "DR_SameCity", function (resAutoObject) {
                        if (resAutoObject.resMsg.code != 200) {
                            autologger.logs(resAutoObject.resMsg.code, "execute [GetStorageViewByProtectLevel] for [DR_SameCity] is FAIL!", resAutoObject);
                            maincallback(resAutoObject);
                        } else
                            callback(null, resAutoObject);
                    })
    ...
    async.mapSeries(StorageViewNames, function (StorageViewName, callback) {
        ...
        GetStorageView(arrayInfo, 'cluster-1', StorageViewName, function (svview_result) {
            ...
            if (svview_result.code == 404) {
                ...
                callback("svview_result.code != 200", svview_result);
            } else {
                callback(null, svview_result);
            }
        })
    }, function (err, result) {
        if (err) {
...
function GetStorageVolumes(arrayInfo, clustername, callback) {
    ...
    if ( test == false ) {
        ...
        var DataFilename = '/tmp/volumes-'+clustername+'.json';
        var response = require(DataFilename);
        callback(response);
    }
}
