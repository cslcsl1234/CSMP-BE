...
function CapacityProvisingServiceV2(AutoObject, maincallback) {
    ...
    // get the two physical array info;
    ...
    async.waterfall(
        [
            // 0-0. VMAX: Add alloca lun in the two physical 
            /* result structure
    {
    "id_sync_flag": false,
    "arrays": [
...
*/
            /*
            function (callback) {
                var DeviceCount = AutoObject.request.count;
                var DeviceCapacityGB = AutoObject.request.capacity;
                ...
                switch (arrayType) {
                    case 'VMAX':
                        VMAX.SyncDeviceID(physicalArrayInfos, function (result) {
                            ...
                            if ( result.code == 200 ) callback(null, result);
                            else callback(result.code,result);

                        })
                        break;

                    case 'Unity': 
                        callback(null, physicalArrayInfos); 
                        break;
            ...
            */
            ...
            // 0. array re-discover in call cluster in VPLEX
            function (arg1, callback) {
                autologger.logger.info("Begin execute step [ ClaimAllStorageVolume ] !");
