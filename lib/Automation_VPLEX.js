...
function CreateVirtualVolumeService(stepgroupname, vplexinfo, backend_array, volname, capacityGB, clustername, StorageView, CGName) {
    ...
    switch (backend_array.array_type) {
        ...
        case "Unity":
            ...
            break;
        default:
            logger.info("not support array type " + backend_array.array_type);
            break;
    }
    ...
    if (clustername != 'cluster-1' && clustername != 'cluster-2') {
        logger.error(`DataError: ${clustername} is not 'cluster-1' or 'cluster-2'.`);
        return [];
    }
...
function CapacityProvisingService(AutoObject, maincallback) {
    ...
    logger.info("Begin execute service [ CapacityProvisingService ] !"); 
    ...
    async.waterfall(
        [
            ...
            function (callback) {
                logger.info("Begin execute step [ ClaimAllStorageVolume ] !");
            ...
            function (arg, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo ] !");
            ...
            function (AutoObject, callback) {
                logger.info("Begin execute step [ ChooseVirtualVolumes ] !");
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ GenerateActionParamaters ] !");
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ Execute Automateion function for each action ] !");
                // -----------------------------------------------------------
...
function CapacityProvisingServiceTEST(AutoObject, maincallback) {
    ...
    logger.info("CapacityProvisingServiceTEST begin")
    ...
    async.waterfall(
        [
            ...
            function (info, callback) {
                ...
                if (isfind == false) {
                    logger.info("Warnning: Prod volume not find assignStorageView Operstion!");
                }
                if (VirtualVolumes == undefined) {
                    logger.info("Warning: not find virtual volume in assignStorageView Operstion!")
                }
...
function VPlexProvising(paramaters, maincallback) {
    ...
    logger.info("VPlexProvising begin")
    ...
    logger.info(physicalArrayInfos);
    async.waterfall(
        [
            // STEP 1: Create Prod physical device in 2 backend physical array behind VPLEX Metro.
            // 1.1 Combind the create paramater.
                    ...
                    switch (arrayType) {
                        ...
                        case 'Unity':
                            ...
                            for (var j = 1; j <= request.count; j++) {
                                ...
                                //var volName = UNITY.GenerateVolName(item, request, timestamp) + volno;
                                logger.info(request.basevolname)
                                logger.info(eval(" `${request.appname}`"));
    ...
    async.waterfall(
        [
            ...
            function (AutoObject, callback) {
                AutoObjectCheckVaild(AutoObject, function (retAutoObject) {
                    if (retAutoObject.resMsg.code != 200) {
                        logger.info(500, "execute step [ AutoObjectCheckVaildV2 ] is FAIL!", retAutoObject);
                        maincallback(retAutoObject);
                    } else {
                        // 全部检查合规，继续执行下一步。
                        callback(null, AutoObject);
            ...
            function (AutoObject, callback) {
                logger.info("Begin execute step [ ChooseVirtualVolumesV2 ] !");
            ...
            function (AutoObject, callback) {
                logger.info("Begin execute step [ GenerateActionParamatersV2 ] !");
...
function DeployVPLEXBPMN() {
    ; (async () => {
        ...
        setTimeout(() => {
            ...
            zbc.close().then(() => logger.info('All workers closed'))
            callback("aa");
        }, 10000)
    })()
}
...
function CreateExtentHandle(payload, complete) {
    ...
    executeParamater["array"] = arrayInfo;
    logger.info("Begin Extent Handle ----");
    CreateExtent(executeParamater, function (result) {
...
function handler(payload, complete) {
    ...
    logger.info('ZB payload', payload)
    complete(res)
}
...
function ExecuteActions(executeParamaters, arrayInfo, ws, callback) {
    async.mapSeries(executeParamaters, function (executeParamater, subcallback) {
        logger.info("Execute action: [ " + executeParamater.method + " ]. depend on action: [" + executeParamater.DependOnAction + "].");
...
function ComponeAutoInfo(AutoObject, maincallback) {
    ...
    async.waterfall(
        [
            ...
            function (callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.GetStorageViewByProtectLevel for prod ] !");
                // ----------------------------
                // 获取生产StorageView及相关信息 
                // ----------------------------
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.GetStorageViewByProtectLevel for TC ] !");
                // ----------------------------
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.GetStorageViewByProtectLevel for Backup ] !");
                // ----------------------------
            ...
            function (AutoObject, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.GetStorageViewByProtectLevel for Same City DC Protect ] !");
                // ----------------------------
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.GetConsistencyGroup ] !");
            ...
            , function (AutoObject, callback) {
                logger.info("Begin execute step [ ComponeAutoInfo.ParserMasterArray&SecondaryArray ] !");
                // ---------------------------------------------------------------------
            ...
            , function (AutoObject, callback) {
                ...
                async.mapSeries(clusters, function (clustername, subcallback) {
                    logger.info("Begin execute step [ ComponeAutoInfo.GetStorageVolumes for " + clustername + " ] !");
...
function ChooseVirtualVolumes(AutoObject, maincallback) {
    ...
    //logger.info(claimedStorageVolumes);
...
function ChooseVirtualVolumesV2(AutoObject, maincallback) {
    ...
    logger.info("claimedStorageVolumeList=" + JSON.stringify(claimedStorageVolumeList));
...
function GenerateActionParamaters(AutoObject) {
    ...
    logger.info("Create Distributed Device Paramater");
    ...
    logger.info("Create Distributed virtual volumes from Distributed Device created ");
        ...
        for (var i in StorageDevices) {
            ...
            switch (item.position) {
                case 'primary':
                    ...
                    logger.info("Primary------------------" + item.name + ',' + tailName + ',' + headName1);
                    //ddDeviceName = ddDeviceName.replace('PPPP', item.name);
                    sourcedevice = 'device_' + item.name;
                    break;
                case 'second':
                    logger.info("Second------------------" + item.name);
                    var name = item.name;
            ...
            function (unclaimedList, callback) {
                ...
                for (var i in unclaimedList) {
                    ...
                    if (StorageVolumeClaimParamater.storagevolume.StorageVolumeName == "") {
                        logger.info("Not Support claim: " + StorageVolumeClaimParamater.storagevolume["storage-array-name"])
                    } else if (StorageVolumeClaimParamater.storagevolume["health-state"] != "ok") {
                        logger.info("Can not claim: " + StorageVolumeClaimParamater.storagevolume["storage-array-name"]);
                    } else {
...
function ClaimStorageVolume(arrayInfo, physicalVolumeList, callback) {
    ...
    //logger.info("TEST111" + JSON.stringify(physicalVolumeList, null, 2));
    async.waterfall(
        [
            // Get All Cluster
            function (callback) {
...
function GetConsistencyGroup(arrayInfo, cluster, cgname, callback) {
    ...
    switch (config.ProductType) {
        ...
        case 'Test':
            ...
            var cgitem = {};
            logger.info('cluster=' + cluster + ',cgname=' + cgname);
        ...
        case 'Prod':
            ...
            CallGet.CallAutoGet(param, function (result) {
                if (result.code == 200) {
                    response.code = 200;
                    response.message = "success";
                    //logger.info(result.response.context[0].attributes);
                    var cgitem = {};
                    for (var i in result.response.context[0].attributes) {
                        var item = result.response.context[0].attributes[i];
                        cgitem[item.name] = item.value;
...
function GetStorageView(arrayInfo, cluster, viewname, callback) {
    ...
    switch (config.ProductType) {
        ...
        case 'Test':
            ...
            logger.info('cluster=' + cluster + ',viewname=' + viewname);
...
function GetStorageVolumes(arrayInfo, clustername, callback) {
    logger.info('debug', "Begin function [ GetStorageVolumes ], paramater [" + clustername + "]");
    logger.info("Begin function [ GetStorageVolumes ], paramater [" + clustername + "]");
                    ...
                    GetStorageArray(arrayInfo, clustername, function (result) {
                        if (result.code == 200) {
                            ...
                            async.mapSeries(storage_array_list, function (arrayItem, subcallback) {
                                logger.info(`### Begin discover the storage array [${arrayItem}] in cluster [${clustername}]`);
...
function GenerateStorateViewName(request, type) {
    ...
    for (var i in Naming) {
        ...
        if (item.SC == SC_VIEW_NAME) {
            //logger.info(item.SC + ' | ' + SC_VIEW_NAME);
            isfind = true;
            naming = item;
            break;
        }
...
function AutoObjectCheckVaild(AutoObject, callback) {
    ...
    //logger.info(JSON.stringify(claimedList));
