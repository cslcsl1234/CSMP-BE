...
function CreateVirtualVolumeService(stepgroupname, vplexinfo, backend_array, volname, capacityGB, clustername, StorageView, CGName) {
    ...
    if (clustername != 'cluster1' && clustername != 'cluster2') {
        callback(501, `DataError: ${clustername} is not 'cluster1' or 'cluster2'.`);
    }
...
function GenerateCreatePhysicalVolumeParamater(stopgroupname, arrayinfo, request, timestamp, maincallback) {
    ...
    var ActionParamaters = [];
    var physicalArrayInfos = arrayinfo["backend-array"];
...
function CapacityProvisingServiceTEST(AutoObject, maincallback) {
    ...
    */
    ...
    for (var i in AutoObject.AutoInfo.RuleResults.ArrayInfo["backend-array"]) {
        var item = AutoObject.AutoInfo.RuleResults.ArrayInfo["backend-array"][i];
    ...
    async.waterfall(
        [
            ...
            function (info, callback) {
                ...
                var LocalCDPInfo = cdpinfo["backend-array"].Local;
                switch (LocalCDPInfo.arraytype) {
                    case "VPLEX":
                        for (var j = 1; j <= request.count; j++) {
                            ...
                            if (LocalCDPInfo["backend-array"].length == 1) {
                                var arrayinfo = LocalCDPInfo["backend-array"][0];
            ...
            function (info, callback) {
                ...
                var remoteCDPInfo = cdpinfo["backend-array"].Remote;
            ...
            function (info, callback) {
                ...
                console.log(RPA_CG);
                var journalBackendArray = AutoObject.AutoInfo.ResourceInfo.CDP["journal-backend-array"];
                    ...
                    for (var i in ActionResult.ProvisedVolumes.Prod) {
                        ...
                        if (isfind == false) {
                            ...
                            for (var i in journaltypes) {
                                ...
                                var volName = `${CGName}_${journaltype}_Log_01`;
                                ...
                                switch (arrayinfo.array_type) {
                                    case 'VPLEX':
                                        ...
                                        var backend_array = arrayinfo["backend-array"][0];
                                        var clustername = 'cluster1';
                                        var sgname = arrayinfo.sgname;
                                        var cgname = arrayinfo.consistencygroup;
                                        var result = CreateVirtualVolumeService(stepgroupname, arrayinfo, backend_array, volName, JournalCapacityGB, clustername, sgname, cgname);
                                        for (var z in result) {
                                            var item = result[z];
...
function VPlexProvising(paramaters, maincallback) {
    ...
    var physicalArrayInfos = [];
    for (var i in arrayInfo["backend-array"]) {
        var item = arrayInfo["backend-array"][i];
        physicalArrayInfos.push(item);
    }
...
function CreateProdVolume(AutoObject, maincallback) {
    ...
    */
    ...
    // get the two physical array info (AutoObject.AutoInfo.RuleResults.ArrayInfo["backend-array"])
    /*
        [
            {
                "array_type":"VMAX",
    ...
    */
    var physicalArrayInfos = AutoObject.AutoInfo.RuleResults.ArrayInfo["backend-array"];
    var arrayType = "Other";
...
function DeployVPLEXBPMN() {
    ; (async () => {
        const zbc = new ZB.ZBClient('192.168.92.154:26500')
