...
var MIB = function (dir) {
    return ({
        ...
        Import: function (FileName) {
            this.ParseModule(FileName.split('//')[1].split('.')[0], fs.readFileSync(__dirname + '/' + FileName).toString());
        },
                                                        ...
                                                        switch (val) {
                                                            ...
                                                            case 'INTEGER':
                                                                ...
                                                                if (Symbols[c1 + 2].indexOf("{") == 0) {
                                                                    ...
                                                                    var integer;
                                                                    var syntax;
                                                                    //logger.info(ModuleName, ObjectName);
                                                                    while (Symbols[c1].indexOf("}") == -1) {
                                                                        c1++;
                                                                        var ok = false;
                                                                        ...
                                                                        if (ok) {
                                                                            ...
                                                                            //logger.info(ModuleName, ObjectName, integer[1], syntax);
                                                                        }
                                                                    }
                                ...
                                while (Symbols[tmp] != ';') {
                                    if (Symbols[tmp] == 'FROM') {
                                        ...
                                        if (!this.Modules[ImportModule]) {
                                            logger.info(ModuleName + ': Can not find ' + ImportModule + '!!!!!!!!!!!!!!!!!!!!!');
                                            logger.info(ModuleName + ': Can not import ', IMPORTS);
                                        }
	    ...
	    if ( OBJECT_IDENTIFIER.indexOf("  ") >= 0 ) OBJECT_IDENTIFIER = OBJECT_IDENTIFIER.replace("  "," ");
...
*/
            this.Import("./RFC_BASE_MINIMUM//SRM-ALERTS-MIB-V1.1.mib");
