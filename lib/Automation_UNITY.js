...
function GenerateCreateDeviceParamater(arrayinfo, volName, sgname, capacityGB) {
 
    var capacityBYTE = capacityGB * 1024 * 1024 * 1024;
    var CreateArrayDeviceParamater = {
        Step: `Create device and assign to sg [ ${sgname} ] in pyhsical array [ ${arrayinfo.unity_sn} ] , arraytype= [ ${arrayinfo.array_type} ]`,
...
function ExecuteAction(executeParamater, callback) {
    ...
    switch (executeParamater.method) {
        case 'CreatePhysicalDevice': 
            var capacity =  executeParamater.capacityByte;
            CreateDevice(executeParamater.arrayinfo, executeParamater.AsignSGName, executeParamater.capacityByte, executeParamater.StorageVolumeName, function (result) {
                if (result.code != 200) { 
                    retMsg.code = result.code;
                    retMsg.data = result.data.msg;
                    retMsg.msg = result.msg;
                
                } else {  
                    retMsg.data = result.data;
                    retMsg.msg = `Unity.CreateDevice is succeedful. array=[${executeParamater.arrayinfo.unity_sn}] sgname=[${executeParamater.AsignSGName}] volname=[${executeParamater.StorageVolumeName}] capacity=[${capacity/1024/1024/1024}(GB)]`;
                }
                callback(retMsg); 
            })
            break;
...
function GenerateVolName (arrayinfo, request, timestamp ) {
    ...
    var volName = `${appname}_unity${subArraySN}_${timestamp}${usedfor}`
    return volName

}

 
function GenerateVolNameV2 (arrayinfo, appname, usedfor, timestamp ) { 
    var arraySN = arrayinfo.unity_sn; 
    var subArraySN = arraySN.substring(arraySN.length-3,arraySN.length); 
    var volName = `${appname}_unity${subArraySN}_${timestamp}${usedfor}`
    return volName 
