            ...
            function (data, callback) {
                ...
                CallGet.CallRPAPut(RPAInfo, url, CreateReplicationSetParamater , function (result) {
                    ...
                    if (result.code == 200) {
                        ...
                        var response = {}
                        response["paramater"] = CreateReplicationSetParamater;
                        response["result"] = result;
                        callback(result.code, response);
                    }
...
function searchVolumeID(clusterinfo, clusterid, volumename ) {
    for ( var i in clusterinfo ) {
        ...
        if ( clusterItem.clusterid == clusterid ) {
            for ( var j in clusterItem.volumes ) {
                var volItem = clusterItem.volumes[j];
                if ( volItem.volumeName == volumename ) {
                    return volItem.id;
                }
            }
        }
    ...
    async.waterfall(
        [
            ...
            function (groupUID, callback) {
                ...
                GetSplitters(rpainfo, "cluster1", function (result) {
                    for (var i in result) {
                        var item = result[i];
                        item["clustername"] = "cluster1";
                    }
                    GetSplitters(rpainfo, "cluster2", function (result2) {
                        for (var i in result2) {
                            var item = result2[i];
                            item["clustername"] = "cluster2";
                        }
...
function CreateConsistencyGroup(rpainfo, createCGParamater, callback) {
    ...
    var clustername_remote = clustername == "cluster1" ? "cluster2" : "cluster1";
...
function CreateCopy(rpainfo, paramater, callback) {
    ...
    var productionCopy;
    if (clustername == 'cluster1' && CopySerial == 0) productionCopy = true;
